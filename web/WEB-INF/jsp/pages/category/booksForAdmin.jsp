        ...
        <form id="bookForm">
         ...
         <sd:Option value="6">Tiếp nhận HS cấp GCN ĐĐK</sd:Option> --%>
                ...
                <tr>
                    ...
                    <td>
                        <sd:TreePicker id="searchDeptTree" getChildrenNodeUrl="departmentAction!getDeptChildrenDataByNode.do"
                                       getTopNodeUrl="departmentAction!getMyDeptRootTree.do"  key="" rootLabel="root" cssStyle="width:100%" />
                        <sd:TextBox cssStyle="width:100% !important; display:none" id="bookForm.officeId"  key=""  name="bookForm.officeId" />
                        <sd:TextBox cssStyle="width:100% !important; display:none" id="bookForm.officeName" key="" name="bookForm.officeName" />
                    </td>
                    <!--                <td colspan="3">
                    <sd:TextBox cssStyle="width:100% !important;"
                ...
                <tr style="text-align: center">
                    <td colspan="4">
                        <sx:ButtonSearch onclick="page.search2();" />
...
<script>
    dijit.byId("searchDeptTree").onPickData = function(item){
        try{
            if(item.id){
                sd._("bookForm.officeId").setValue(item.id);
                sd._("bookForm.officeName").setValue(item.name);
            } else {
                sd._("bookForm.officeId").setValue("");
                sd._("bookForm.officeName").setValue("");
                sd._("searchDeptTree").setValue("");
            }
        }catch(err){
            alert(err.message);
        }
...
<sd:Dialog  id="dlgbookAddEdit" height="auto" width="800"
    ...
    <jsp:include page="booksAddEditForAdmin.jsp" flush="false"></jsp:include>
</sd:Dialog>
...
<script>
    ...
    //Tim kiem khi chon don vi tu khung tim kiem
    page.search2 = function() {
        var officeId = dijit.byId("bookForm.officeId").getValue();
        if(officeId != ""){
    ...
    page.reset = function() {
        ...
        dijit.byId("bookForm.officeId").setValue("");
        dijit.byId("bookForm.officeName").setValue("");
        sd._("searchDeptTree").setValue("");
        // grid.vtReload('voBooks!search.do?', "bookForm",null,null);
    }
    ...
    page.afterShowEditPopup = function(data) {
        ...
        if(customInfo.officeId!=null) {
            dijit.byId("bookFormAddEdit.officeId").setValue(escapeHtml_(customInfo.officeId));
        }
        if(customInfo.officeName!=null) {
            dijit.byId("bookFormAddEdit.officeName").setValue(escapeHtml_(customInfo.officeName));
            sd._("deptTreeFrom").setValue(escapeHtml_(customInfo.officeName));
        }
        if(customInfo.isDefault != null) {
            dijit.byId("bookFormAddEdit.isDefault").setValue(customInfo.isDefault);
        }
