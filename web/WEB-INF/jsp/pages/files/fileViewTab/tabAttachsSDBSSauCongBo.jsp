...
%>
<script>
    createNewRowOfAttach2 = function (attachId, categoryName, attachDes, attachName, tableId, isTemp, objectType) {
        if (attachId == null) {
            attachId = "";
        }
...
//        }
    ...
    uploadNewRow2 = function (attachName, attachId, id) {
...
<div id="lstFileUpload2" style="display: none"></div>
<br/>
<table id="attachTbl2" class="lstTable">
    <tr class="headerRow">
        <th width="5%">STT</th>
...
<script>
    createEmptyAttachs2 = function () {
        ...
        document.getElementById("attachTbl2").appendChild(row);

    };

    page.afterLoadAttach2 = function (data) {
        var obj = dojo.fromJson(data);
        var result = obj.items;
        if (result != null && result.length > 0) {
            for (var i = 0; i < result.length; i++) {
                var item = result[i];
                createNewRowOfAttach2(item.attachId, item.categoryName, item.attachDes, item.attachName, 'attachTbl2', item.isTemp, item.objectType);
            }
        } else {
            createEmptyAttachs2();
        }
    ...
    page.loadInitAttachs2 = function () {
        ...
        if (fileId == null || fileId == "") {
            createNewRowOfAttach2('', '', '', "", 'attachTbl2', '');
            
        } else {
            sd.connector.post("filesAction!loadAttachs2.do?createForm.fileId=" + fileId, null, null, null, page.afterLoadAttach2);
        }
    };
    page.loadInitAttachs2();
</script>