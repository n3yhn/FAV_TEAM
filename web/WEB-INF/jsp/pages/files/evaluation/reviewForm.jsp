    ...
    <table width="100%" class="viewTable" id="tblReviewForm">
        ...
        <tr>
            <td colspan="2" style="text-align: center">
                ...
                <br>
                <sd:Button id="btnLoadCookieRF" key="" onclick="page.getReviewForm();" cssStyle="display:" cssClass="buttonGroup">
                ...
                <sd:Button id="btnSaveCookieRF" key="" onclick="page.setRreviewForm();" cssStyle="display:" cssClass="buttonGroup">
                    <img src="share/images/icons/foward_email.png" height="14" width="14" alt="Xem truoc"/>
                    <span style="font-size:12px">Lưu nháp ND thẩm định</span>
                </sd:Button>
                <sd:Button id="btnClearCookieRF" key="" onclick="page.clearReviewForm();" cssStyle="display:" cssClass="buttonGroup">
                    <img src="share/images/icons/foward_email.png" height="14" width="14" alt="Xem truoc"/>
...
//        }
        msg.confirm("Bạn có chắc chắn về kết quả xem xét này không ?", "Xem xét hồ sơ", onReviewFormSaveNew);
    };

    //hieptq update 160415
    onReviewFormSaveNew = function() {
        if (validateReviewForm()) {
            sd.connector.post("filesAction!onReview.do?" + token.getTokenParamString(), null, "reviewForm", null, afterReviewFormSave);
        }
    };
    ...
    validateReviewForm = function() {
        if (document.getElementById("reviewForm.statusAccept").checked == false && document.getElementById("reviewForm.statusDeny").checked == false && document.getElementById("reviewForm.statusDenyCV").checked == false) {
        ...
        if (document.getElementById("reviewForm.statusAccept").checked == true
            ...
            if (document.getElementById("reviewForm.statusDeny").checked) {
                ...
                if ((dijit.byId("reviewForm.legalL").getValue() == 0 || dijit.byId("reviewForm.legalL").getValue() == -1) && legalContentL.trim().length == 0) {
                ...
                if ((dijit.byId("reviewForm.foodSafetyQualityL").getValue() == 0 || dijit.byId("reviewForm.foodSafetyQualityL").getValue() == -1) && foodSafetyQualityContentL.trim().length == 0) {
                    alert("[Về chỉ tiêu chất lượng an toàn thực phẩm] chưa nhập lý do");
                    dijit.byId("reviewForm.foodSafetyQualityContentL").focus();
                    return false;
                }
                if ((dijit.byId("reviewForm.effectUtilityL").getValue() == 0 || dijit.byId("reviewForm.effectUtilityL").getValue() == -1) && effectUtilityContentL.trim().length == 0) {
            ...
            if (document.getElementById("reviewForm.statusDeny").checked == true || document.getElementById("reviewForm.statusAccept").checked == true) {
                ...
                if (leaderId == -1 || leaderId == "" || leaderId == null) {
                    ...
                    dijit.byId("reviewForm.leaderApproveId").setValue(-1);
...
//            }
    ...
    onCloseReviewForm = function() {
        ...
        document.getElementById("reviewForm.statusAccept").checked = false;
