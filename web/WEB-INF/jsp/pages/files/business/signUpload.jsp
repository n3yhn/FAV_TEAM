    ...
    <table width="100%;" cellpadding="0px" cellspacing="5px" style="table-layout: fixed; word-break: break-all;">
        <tr>
            ...
            <td width="70%" id="tdBill20">
                ...
                <sx:upload_one extension="*.pdf,*.doc,*.docx,*.zip,*.rar, *.docx, *.xls, *.xlsx"
                               auto="true" id="sendForm.attachFileNew"
                               action="uploadiframe!uploadFile.do?id=sendForm.attachFileNew" maxSize="8">
                </sx:upload_one>
...
<script>
    ...
    {
        msg.confirm('Nội dung file hồ sơ tải lên phải chính xác với nội dung đã khai báo trên hệ thống , bạn có chắc chắn thực hiện hành động này ?', '<sd:Property>Cảnh báo</sd:Property>',  page.saveUpload);
    };
    page.saveUpload = function()
    {
        ...
        if (billPath != null && billPath.trim().length > 0)
        {
            ...
            {
                sd.connector.post("filesAction!onUploadFileSign.do?fileId=" + workingFileId + "&billPath=" + billPath + "&" + token.getTokenParamString(), null, null, null, page.afterSavePaymentNew);
            }
            else
            {
