...
<script type="text/javascript">
    ...
    createNewMainlyTargetRow = function(mainlyTargetId, targetName, unitId, publishLevel) {
...
<table class="editTable">
    ...
    <tr>
        <td width="25%"><sd:Label>Trạng thái</sd:Label></td>
            <td width="25%">
            <sd:TextBox key="" 
                        id="createForm.detailProduct.productStatus" 
                        name="createForm.detailProduct.productStatus" 
                        maxlength="255"
                        cssStyle="width:98%"
                        trim="true"/>
        </td>
        <td width="25%"><sd:Label>Màu sắc</sd:Label></td>
            <td width="25%">
            <sd:TextBox key="" 
                        id="createForm.detailProduct.productColor" 
                        name="createForm.detailProduct.productColor" 
                        maxlength="255" 
                        cssStyle="width:98%"
                        trim="true"/>
        </td>
    ...
    <tr>
        <td width="25%"><sd:Label>Mùi vị</sd:Label></td>
            <td width="25%">
            <sd:TextBox key="" 
                        id="createForm.detailProduct.productSmell" 
                        name="createForm.detailProduct.productSmell" 
                        cssStyle="width:98%"
                        maxlength="255"
                        trim="true"/>
        </td>
...
//        }
    ...
    page.afterLoadMainlyTarget = function(data) {
        ...
        if (result != null && result.length > 0) {
            for (var i = 0; i < result.length; i++) {
                var item = result[i];
                createNewMainlyTargetRow(item.mainlyTargetId, item.targetName, item.unitId, item.publishLevel);
            }
        }
    ...
    page.afterLoadMainlyTargetCopy = function(data) {
        ...
        if (result != null && result.length > 0) {
            for (var i = 0; i < result.length; i++) {
                var item = result[i];
                createNewMainlyTargetRow("", item.targetName, item.unitId, item.publishLevel);
            }
        }
    };
    ...
    page.showFee = function()
    {
        ...
        var tpcnId = "${tpcnId}";
        var priceDBT = "${priceTPDB}";
        var priceTPCN = "${priceTPCN}";
        var priceETC = "${priceETC}";
        var tlId = "${tlId}";
        var dem = 0;
