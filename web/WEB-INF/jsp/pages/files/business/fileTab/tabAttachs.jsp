...
<script type="text/javascript">
    ...
    page.validateAttachData = function () {
        ...
        try
        {
            ...
            if (countCheckLabel > 3) {
                alert("Số lượng tệp đính kèm xuất cùng bản công bố vượt quá 3 tệp. Vui lòng ghép các file nhãn hoặc RNI thành 1 tệp duy nhất trước khi tải lên.");
                return false;
            }
            var lstAttachLabel = "";
            for (j = 1; j < rows.length; j++) {
    ...
    page.validateAttachData2NotRequireRNI = function () {
        var countCheckLabel = 0;
        var countValidate = page.checkCountFile();
        var tabs = dijit.byId("files_tab");
        var panel = dijit.byId("tab.attachs");
