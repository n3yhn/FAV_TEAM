...
//                                    tdUploadAtt.appendChild(button);
    ...
    <div id="lstFileUpload2"></div>
    <br/>
    <br/>
    <table id="excelTbl1" class="lstTable">
        <tr class="headerRow">
            <th width="5%">STT</th>
    ...
    <div>
    <sx:ButtonAddCategory onclick="createNewRowOfAttach('','','','','excelTbl1');"/>
    <sd:Button id="btnImportExcel" key="" onclick="page.import();" cssStyle="display:" cssClass="buttonGroup">
        <img src="${contextPath}/share/images/icons/DownArrow.png" height="14" width="18">
        <span style="font-size:12px">Nhập thông tin</span>
    </sd:Button>
...
<script type="text/javascript">
    page.renameAttachIndexs = function() {
        var table = document.getElementById("excelTbl1");
    ...
    page.import = function() {
        ...
        sd.connector.post("filesAction!importFileFromExcel.do?attachId=" + workingAttachId + "&createForm.fileType=66701", "createFileDiv", null, null, page.afterLoadExcel);
    };
    page.afterLoadExcel = function()
    {
    ...
    page.afterLoadAttach = function(data) {
        ...
        if (result != null && result.length > 0) {
            for (var i = 0; i < result.length; i++) {
                var item = result[i];
                createNewRowOfAttach(item.attachId, item.categoryName, item.attachDes, item.attachName, 'excelTbl1');
            }
        }
    };
    ...
    page.loadInitAttachs = function() {
        ...
        document.getElementById("lstFileUpload2").innerHTML = fileLst;
        var fileId = dijit.byId("createForm.fileId").getValue();
        if (fileId == null || fileId == "") {
            //createNewRowOfAttach('', '', '', "", 'excelTbl1');
        } else {
            //sd.connector.post("filesAction!loadAttachs.do?", null, null, null, page.afterLoadAttach);
        }
    };
    ...
    page.userAttachsDlg = function() {//        
        ...
        lstUserAttach2 = dijit.byId("lstUserAttach1").getValue();
        document.getElementById("list-selected-itemlstUserAttach1").innerHTML = "";
        dijit.byId("lstUserAttach").setValue("");
    };
    page.loadInitAttachs1 = function() {
        var lstUserAttach1 = dijit.byId("lstUserAttach1").getValue();
    ...
    page.afterLoadAttach1 = function(data) {
        ...
        if (result != null && result.length > 0) {
            ...
            for (var i = 0; i < result.length; i++) {
                var item = result[i];
                var size = getRowLengthOfTable("excelTbl1") - 1;
                ck = true;
