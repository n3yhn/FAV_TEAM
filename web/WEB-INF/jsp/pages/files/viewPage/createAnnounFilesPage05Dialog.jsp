...
<div class="buttonDiv" id="buttonDiv">
    ...
    <sd:Button id="btnComparison" key="" onclick="page.showViewComparisonForm();" cssStyle="display:none" cssClass="buttonGroup">
        ...
        <span style="font-size:12px">Kết quả đối chiếu hồ sơ</span>
    </sd:Button>
    ...
    <!--Xem truoc cong van SDBS sau co bo-->
    <sd:Button key="" onclick="page.downloadFileDetails2();" >
        <img src="${contextPath}/share/images/icons/Answer.png" height="14" width="18">
        <span style="font-size:12px">Xem trước công văn SĐBS sau công bố</span>
    </sd:Button> 
...
//         if ((status == 3 || status == 4 || status == 5 || status == 6 || status == 8 || status == 9 || status == 14 || status == 15 || status == 19 || status == 17 || status == 20 || status == 21) && (viewType == 4 || viewType == 2 || viewType == 3)) {
    ...
    page.showBtnCheckCA = function () {
        var elementContentSigned = document.getElementById("createForm.contentSigned");
        if (elementContentSigned != null) {
            var contentSigned = dijit.byId("createForm.contentSigned").getValue();
    ...
    page.downloadFileDetails2 = function () {
        var fileId = dijit.byId("createForm.fileId").getValue();
        document.location = "exportWord!onExportFileDetails.do?fileId=" + fileId + "&&isPre=1";
    };
...
//    page.showCheckCa = function() {//hiển thị đối chiếu CA
//        var status = dijit.byId("createForm.status").getValue();
//        var userSigned = dijit.byId("createForm.userSigned").getValue();
//        //alert(contentSign1);
//        if (status != 0 && userSigned != "") {
//            
//            dijit.byId("btnCheckCA").domNode.style.display = "";
//        }
//    };
//
//    page.showCheckCa();
    //250214 binhnt53 comparison    

    page.showComparisonDlg = function () {
        var fileId = dijit.byId("createForm.fileId").getValue();
        dijit.byId("createForm.fileId").setValue(fileId);
...
//    page.viewFlowFileTitleAndContents = function() {//Lanh dao sua noi dung cong van SDBS sau cong bo
//        var editTitleAndContentFile = dijit.byId("editTitleAndContentFile");
//        var filesSourceID = dijit.byId("createForm.fileId").getValue();
//        editTitleAndContentFile.show();
//        page.getProcess3(filesSourceID);
//    };
