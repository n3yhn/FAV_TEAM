...
<div class="buttonDiv" id="buttonDiv">
    ...
    <sd:Button id="btnEvaluateLeaderFiles" key="" onclick="page.getCommentEvaluateLeaderForm();" cssStyle="display:none" cssClass="buttonGroup">
        <img src="share/images/edit.png" height="14" width="14" alt="Kết luận thẩm định"/>
        <span style="font-size:12px">Phó phòng kết luận thẩm định</span>
    </sd:Button>
    <sd:Button id="btnCommnetEvaluation" key="" onclick="page.showComment();" cssStyle="display:none" cssClass="buttonGroup">
        ...
        <span style="font-size:12px">Trao đổi Ý kiến</span>
    </sd:Button>
    ...
    <sd:Button id="btnRejectReceived" key="" onclick="page.rejectReceived();" cssStyle="display:none" cssClass="buttonGroup"> 
        ...
        <span style="font-size:12px">Từ chối tiếp nhận hồ sơ</span>
    </sd:Button>
    ...
    <sd:Button id="btnViewFLow" key="" onclick="page.viewFlow();" cssStyle="display:none" cssClass="buttonGroup">
        ...
        <span style="font-size:12px">Xem luồng hồ sơ</span>
    </sd:Button>
    <sd:Button id="btnViewFileSource" key="" onclick="page.viewFlowFileSource();" cssClass="buttonGroup">
        <img src="share/images/icons/process_icon.png" height="14" width="14" alt="Luồng hồ sơ gốc"/>
        <span style="font-size:12px">Luồng hồ sơ gốc</span>
    </sd:Button>
    <sd:Button id="btnCheckCA" key="" onclick=" page.checkCAValidNew();" cssStyle="display:none" cssClass="buttonGroup">
        <img src="share/images/icons/process_icon.png" height="14" width="14" alt="Luồng hồ sơ"/>
        <span style="font-size:12px">Đối chiếu CA</span>
    </sd:Button>   
    <sd:Button id="btnEvaluationDetails" key="" onclick="page.showEvaluateDetailsForm();" cssStyle="display:none" cssClass="buttonGroup">
        ...
        <span style="font-size:12px">Xem kết luận thẩm định</span>
    </sd:Button>
    ...
    <sd:Button id="btnComparison" key="" onclick="page.showViewComparisonForm();" cssStyle="display:none" cssClass="buttonGroup">
        ...
        <span style="font-size:12px">Xem kết quả đối chiếu hồ sơ</span>
    </sd:Button>
    ...
    <sd:Button id="downloadPdf" key="" onclick="page.downloadFileSignNew();" cssStyle="display:none" cssClass="buttonGroup">
        ...
        <span style="font-size:12px"><sd:Property>Xem bản công bố doanh nghiệp tải lên</sd:Property></span>
    </sd:Button>
...
<form id="createForm" name="createForm">   
    <table width="100%" cellspacing="5" cellpadding="0" border="0" class="viewTable">
        <tr style="display: none">
            <td style="padding-right: 2em;text-align: center"> 
                ...
                <sd:TextBox key="" id="createForm.contentSigned" name="createForm.contentSigned" value="" cssStyle="display:none"/>
    ...
    <sd:Tab id="files_tab" height="750px" width="100%">
        <sd:ContentPane key="Bản công bố" id="tab.annoucement">
            <div id="tabAnnouncementEditDiv" style="overflow: auto;">
                <jsp:include page="../fileViewTab/tabEditAfterAnnounced.jsp"/>
            </div>
        </sd:ContentPane>    
...
<sd:Dialog  id="processCommentDlg" height="auto" width="1000px"
...
<sd:Dialog id="lookupProcessFileSourceDlg" key="Xem luồng xử lý hồ sơ gốc" width="70%">
    <div style="overflow: auto; max-height: 500px">
        <jsp:include page="../lookup/lookupProcessFileSourceDlg.jsp" />
    </div>
</sd:Dialog>    
...
<script>
    page.showReceivedButton = function() {//hien thi nut tiep nhan ho so - voi van thu
        ...
        var isFee = dijit.byId("createForm.isFee").getValue();
        if ((status == 1 || status == 18) && viewType == 1 && isFee == 1) {
...
//         if ((status == 3 || status == 4 || status == 5 || status == 6 || status == 8 || status == 9 || status == 14 || status == 15 || status == 19 || status == 17 || status == 20 || status == 21) && (viewType == 4 || viewType == 2 || viewType == 3)) {
    ...
    page.checkAnnouncementReceiptPaper = function() {//kiem tra ton tai giay cong bo
        var fileId = dijit.byId("createForm.fileId").getValue();
        sd.connector.post("filesAction!oncheckAnnouncementReceiptPaper.do?createForm.fileId=" + fileId, null, null, null, page.aftercheckAnnouncementReceiptPaper);
    };
    page.aftercheckAnnouncementReceiptPaper = function(data) {//sau khi kiem tra ton tai cong bo
        var obj = dojo.fromJson(data);
        var result = obj.items;
        var result0 = result[0];
        if (result0 == "3") {
            dijit.byId("btnAnnouncementReceiptPaperSign").domNode.style.display = "none";
        } else {
            dijit.byId("btnAnnouncementReceiptPaperSign").domNode.style.display = "";
        }
    };
    ...
    afterLoadViewComparisonForm = function(data) {
        ...
        document.getElementById("filesForm.comparisonContent").innerHTML = item[1];
    ...
    page.showAnnouncementReceiptPaperDlg = function() {//show form khai báo giấy tiếp nhận công bố hợp quy
        ...
        var viewType = dijit.byId("createForm.viewType").getValue();
        if (viewType == 4) {
            dijit.byId("btnExportAnnounment").domNode.style.display = "";
        }
    ...
    page.viewFlowFileSource = function() {//xem luong
        var lookupProcessFileSourceDlg = dijit.byId("lookupProcessFileSourceDlg");
        var filesSourceID = dijit.byId("createForm.fileId").getValue();
        lookupProcessFileSourceDlg.show();
        page.getProcess2(filesSourceID);
    };
...
//                
                    ...
                    page.checkAnnouncementReceiptPaper();
