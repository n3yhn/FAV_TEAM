...
<script type="text/javascript">
    ...
    page.formatAction = function(inData) {
        ...
        if (item != null) {
            ...
            if (item.status == 10 || item.status == 13) {
                url += " | <img src='share/images/alertGui.png' width='17px' height='17px' title='Trình ký giấy tiếp nhận công bố hợp qui' onClick='page.showSigningDlg(" + row + ");' />";
                url += " | <img src='share/images/edit.png' width='17px' height='17px' title='Sửa lại giấy tiếp nhận công bố hợp quy' onClick='page.showProvideEditDlg(" + row + ");' />";
            }
            if (item.status == 6)
            {
                url += " | <img src='share/images/edit.png' width='17px' height='17px' title='Soạn thảo giấy tiếp nhận công bố hợp quy' onClick='page.showProvideCreateDlg(" + row + ");' />";
            }
        }
        url += "</div>"
        return url;
    ...
    page.formatStatus = function(inData) {
        ...
        if (item != null) {
            ...
            if (item.status != 13)
            {
                url = "<a href='#' onClick='showApprovedResult(" + row + ")'>" + url + "</a>"
            } else {
                ...
                <td>
                    <div style="width:100%;">
                        <sd:DataGrid id="filesGrid"
                            ...
                            <sd:ColumnDataGrid  key="Kết quả phê duyệt" formatter="page.formatStatus" get="page.getRow"
...
<sd:Dialog  id="approveViewDlg" height="auto" width="600px"
            key="Kết quả phê duyệt" showFullscreenButton="false"
            >
    <jsp:include page="approveFormView.jsp" flush="false"></jsp:include>
</sd:Dialog>
...
<sd:Dialog  id="signingDlg" height="auto" width="600px"
            key="Trình ký giấy tiếp nhận công bố hợp qui" showFullscreenButton="false"
            >
    <jsp:include page="signingDialog.jsp" flush="false"></jsp:include>
</sd:Dialog>
...
<script type="text/javascript">
    ...
    page.search = function() {
        dijit.byId("filesGrid").vtReload('filesAction!onSearchFileToSigning.do', "searchForm");
    };
    ...
    showApprovedResult = function(row) {//show phe duyet
        ...
        document.getElementById("approveFormView.staffRequest").innerHTML = escapeHtml_(file.leaderRequest);
        dijit.byId("approveViewDlg").show();
    };
    ...
    page.showProvideCreateDlg = function(row) {//show form khai báo giấy tiếp nhận công bố hợp quy
        var file = dijit.byId("filesGrid").getItem(row);
        sd.connector.post("filesAction!loadProvidePage.do?provideForm.fileId=" + file.fileId, "createDiv", null, null, afterLoadForm);
    };
    page.showProvideEditDlg = function(row) {//show form khai báo giấy tiếp nhận công bố hợp quy
        var file = dijit.byId("filesGrid").getItem(row);
        sd.connector.post("filesAction!loadProvidePage.do?provideForm.fileId=" + file.fileId, "createDiv", null, null, afterLoadForm);
    };
    page.showSigningDlg = function(row) {//show form khai báo giấy tiếp nhận công bố hợp quy
        var file = dijit.byId("filesGrid").getItem(row);
        dijit.byId("signingForm.fileId").setValue(file.fileId);
        dijit.byId("signingDlg").show();
    };
