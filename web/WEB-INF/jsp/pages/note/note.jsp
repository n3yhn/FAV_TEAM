...
<div class="notes" style="width:100%;background-color: #DDD; height: 300px">
    <s:if test="#request.type == 'menu'">
        <table style="width:100%;border-spacing: 0px;">
            <tr>
                <td class="headerRow">Tạo note:
                    <img id="add_note" class="cursor-pointer" src='${contextPath}/share/note/images/note.png' width='14px' height='14px' title="tạo note" />
                </td>
            </tr>
        </table>
    </s:if>
...
<script type="text/javascript">
    ...
    var edited = function(note) {
        ...
        sd.connector.post("noteAction!update.do?"+token.getTokenParamString(), null, "noteForm", null, null);
    ...
    var created = function(note) { 
        ...
        sd._("noteForm.posY").setValue(note.pos_y);
        sd._("noteForm.text").setValue(note.text);
        sd.connector.post("noteAction!insert.do?"+token.getTokenParamString(), null, "noteForm", null, function(data) {
    ...
    var deleted = function(note) {
        ...
        sd._("noteForm.posY").setValue(note.pos_y);
        sd._("noteForm.text").setValue(note.text);
        sd.connector.post("noteAction!delete.do?"+token.getTokenParamString(), null, "noteForm", null, null);
    }
    ...
    var resized = function(note) {
        ...
        sd._("noteForm.posY").setValue(note.pos_y);
        sd._("noteForm.text").setValue(note.text);
        jQuery('#textarea-note-'+ note.id).css({
            'width': jQuery("#note-" + note.id).width() - 5, 
            'height': jQuery("#note-" + note.id).height() - 5,
            'resize': 'none'
        });
        sd.connector.post("noteAction!update.do?"+token.getTokenParamString(), null, "noteForm", null, null);
    }					
