...
<script type="text/javascript">
    ...
    getAttacthFileView = function(objectId, objectType, id) {
        sd.connector.post("uploadiframe!getAttachFile.do?objectId=" + objectId + "&objectType=" + objectType + "&id=" + id, null, null, null, callBackGetAttachView);
    };
    ...
    callBackGetAttachView = function(data) {
        data = dojo.fromJson(data);
        var id = data.customInfo;
        var result = data.items;
        var ids = result[0];
        var names = result[1];
        for (var i = 0; i < ids.length; i++) {
            uploadNewRowReadonly(names[i], ids[i], id, true);
        }
    };
        ...
        msg.alert("File đã chọn không đúng định dạng cho phép (" + allExt + ")", "<sd:Property>confirm.title</sd:Property>");
            ...
            page.isValidSize = function(size) {
                ...
                if (size / 1024 / 1024 > maxSizeMB) {
                    msg.alert("File đã chọn dung lượng không được vượt quá 3 MB", "<sd:Property>confirm.title</sd:Property>");
                                return false;
                            }
                            return true;
                        };
