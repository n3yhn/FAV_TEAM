...
<script type="text/javascript">
    ...
    uploadNewRowImp = function(name, attachId, id) {
        if (id == "upload.xsl") {
            clearAttFile(id);
        }
    ...
    callBackGetAttach = function(data) {
        ...
        for (var i = 0; i < ids.length; i++) {
            uploadNewRowImp(names[i], ids[i], id, true);
        }
    };
                ...
                onstart: function(filename)
                {
                    ...
                    if (!files) {//for IE
                        if (file.size > 0) {
                            ...
                            if (!page.isValidExtImp(fileName))
                                return false;
                        }
                    } else {
                        if (document.getElementById('${fn:escapeXml(id)}').files.length > 0) {
                            ...
                            //if(!page.isValidExtImp(fileName)) return false;
                            /*
                             if('${fn:escapeXml(id)}' == "digitizingForm.uploadFile"){
                             if(!page.isValidExtImp(fileName)) return false;
                             }
                             */
                            if (!page.isValidExtImp(fileName))
                                return false;
                            if (!page.isValidSizeImp(size))
                                return false;
                        }
                    }
...
<script>
    ...
    page.isValidExtImp = function(fileName) {
        ...
        msg.alert("File đã chọn không đúng định dạng cho phép (" + "*." + allExt + ")", "<sd:Property>confirm.title</sd:Property>");
            ...
            page.isValidSizeImp = function(size) {
                var maxSizeMB = 20;
