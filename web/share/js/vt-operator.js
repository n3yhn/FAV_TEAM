    ...
    checkHTTPStatus : function(/*d.ioArgs*/ ioArgs, /*Var*/ sth) {
        try {
            ...
            if(status == 200) {
                ...
                dlg.show();
                dlg.vt_disableSwitchSizeFeature();*/
                //alert("Đã hết phiên làm việc hoặc có lỗi xảy ra, bạn vui lòng đăng nhập lại");
                document.location.href = sd.operator.logoutURL;                
            }
            else {
...
//            breadCumbDiv.innerHTML = breadCumbStr;
    ...
    displayErrorWhileSendingRequest : function(/*Dojo.Error*/ err) {
        try{
            ...
            if(this.getDebugMode() == true) {
                ...
                ifrDoc.close(); //LongH says: clear existed content
                ...
                div.innerHTML = "Có lỗi xảy ra";

                //[ Show entire error content
                ifrDoc.close(); //LongH says: clear existed content
                ifrDoc.write("Rất tiếc ! trong quá trình xử lý có lỗi xảy ra, hãy liên hệ với quản trị để làm rõ.");
                //] Show entire error content
    ...
    displayWaitScreen : function( /*Boolean*/ state ) {
    ...
    //[ LongH says: not in used anymore @25may11
