...
dojo.declare("vt.dataGrid.vtDataGrid",[dijit._Widget, dijit._Templated],{
    ...
    extendGrid:function(){
        ...
        grid._refresh = this._gridRefresh;//Linhntn for change refresh function
    ...
    vtSort:function(){
        this._lastScrollTop=this.scrollTop;this._fetch(0,true,0); //Linhntn sort de grid khong bi co lai
    },
    ...
    _doHeaderClick:function(e) { // DungDV: fix bug bao loi khi boi den header tren IE
        if(e.cell !== null && e.cell !== undefined && (e.cell.type === null || e.cell.type === undefined
    ...
    _vtSetting:function() {
        try{
            ...
            var html = "<span style=\"color:#0B0B3B;\"><b>Columns</b></span>";
            ...
            dialog.titleNode.innerHTML = "Settings";
    ...
    vtCheckAll: function(){
        ...
        var _this = this;
        try{
            _this.selection.selectRange(0,this.rowCount);
            this._checkAll = true;
            ...
            if(this.serverPaging !== undefined && this.serverPaging == true){
             ...
             //alert(store.fetch);
               if ((pageNumber !== undefined)&&(pageNumber !== null)){
                    var currentGridRow = (pageNumber-1) * this.rowsPerPage;//Linhntn for go to Page when reload
                    this._refresh(true,currentGridRow);}
                else {
                    var currentGridRow = this.currentPage * this.rowsPerPage;//Linhntn for go to Page when reload
                    this._refresh(true,currentGridRow);
                }
    ...
    _gridRefresh:function(isRender, currentRow){
		this._clearData();
		this._fetch(0, isRender, currentRow);
	},
        ...
        if (arrProps){
            ...
            for (prop in item){
                if ((prop != "_0") && (prop != "_R") && (prop!="_RS")
                    ...
                    if(item[prop]) {//04/11/2011 Linhnt for check item is null
                        if (!dojo.isObject(item[prop][0])){
                            if (!outContent[ propForPost]){
                                outContent[ propForPost] = [];
                            }
        ...
        if(this.doClientPaging && this._lastServerQuery !== null &&
            ...
            if(this._jsonFileUrl){
                ...
                var xhrHandler = xhrFunc({
                    ...
                    failOk: this.failOk,
                    sync: true, //Linhntn for block util data is returned
                    //LongH@6Aug11
