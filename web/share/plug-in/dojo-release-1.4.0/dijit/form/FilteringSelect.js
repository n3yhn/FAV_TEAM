...
//getItemFollowIndex Linhntn 03/02/2012
getItemFollowIndex:function(index){
     var OptionsTag = (this.store)? (this.store.root)? this.store.root.options:null:null;
        ...
        if (OptionsTag){
            ...
            return OptionsTag[index];
            }catch(e){
                alert(this.id, "Set index: '"+index+"' th?t b?i", e);
            }
        }else if (Items){
            try{
            ...
            alert(this.id,"Set index: '"+index+"' th?t b?i. Kh�ng th?y bi?n store c?a control",null)
        }
    }
...
//[Linhntn 03/02/2012 for check value not in list option
if ((_1[0]!== undefined)&& (_1[0] !==null)){
    if(!_2&&_1.length&&this.store.getIdentity(_1[0])!=this._lastQuery){
        return;
        }
}

if ((_1[0] === undefined)||(_1[0] ===null)){
    //this.attr("item",this.getItemFollowIndex(0),_3); 
    return;
}
//]Linhntn
if(!_1.length){
